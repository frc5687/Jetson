...
# Natural number shape definitions for reshaping matrices
# TODO test other possible shapes
N2 = 9000, 2
N3 = 300, 3
# Create a point cloud object and visualizer object it will be handy later
...
careful not to do much work on this thread to avoid datalso be
...
try:
    ...
    while True:
        ...
        if valid:
            ...
            # compute the disparity on the center of the frames and convert it to a pixel disparity (divide by DISP_SCA
            # LE=16)
            disparity = stereo.compute(center_undistorted["left"], center_undistorted["right"]).astype(
                np.float32) / 10.0  # Was 16
            # re-crop just the valid part of the disparity
            disparity = disparity[:, max_disp:]
            # plt.plot(disparity[0])
            # plt.show()
            ...
            array = np.resize(disparity, (90000, 3))

            xyz = array
            pcd = o3d.geometry.PointCloud()
            pcd.points = o3d.utility.Vector3dVector(xyz)
            o3d.io.write_point_cloud("./data.pcd", pcd)
